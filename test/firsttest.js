...
describe("manual Proxy test",function(){
    ...
    it("Should set and get value via proxy", async function () {
        const proxyAsLogicV1 = LogicV1.attach(await proxyInstance.getAddress());
        console.log(await proxyAsLogicV1.interface.fragments);
        await proxyAsLogicV1.setX(42);
        console.error(await proxyAsLogicV1.getX());
        expect(await proxyAsLogicV1.getX()).to.equal(42);
        await proxyAsLogicV1.increament();
        expect(await proxyAsLogicV1.getX()).to.equal(43);
    });
    ...
    //     await proxyInstance.upgrade(logicV2Instance.address);

    //     // Attach proxy to new LogicV2 ABI
    //     const proxyAsLogicV2 = LogicV2.attach(proxyInstance.address);
    //     await proxyAsLogicV1.setX(42);
    //     // Verify state persists
    //     expect(await proxyAsLogicV2.getValue()).to.equal(42);

    //     await proxyAsLogicV1.increament();
    //     expect(await proxyAsLogicV2.doubleValue()).to.equal(52);
    // });